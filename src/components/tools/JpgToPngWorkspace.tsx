...
import { Download, RotateCcw, CheckCircle2, AlertCircle, Loader2, ArrowRightLeft } from 'lucide-react';
    ...
    <div className="w-full max-w-4xl mx-auto p-6 md:p-8 bg-white dark:bg-[#121829] border border-slate-200 dark:border-slate-800 rounded-3xl shadow-xl space-y-6">
      ...
      {(status === 'file_selected' || status === 'processing') && file && (
        <div className="space-y-6">
          ...
          <div className="p-5 rounded-3xl bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-800 flex items-center justify-center space-x-4 text-sm font-extrabold text-slate-700 dark:text-slate-300">
            <span>JPG / JPEG</span>
            <ArrowRightLeft className="w-5 h-5 text-blue-600 dark:text-indigo-400" strokeWidth={1.75} />
            <span className="text-blue-600 dark:text-indigo-400 font-black">Lossless PNG</span>
          </div>
        ...
        <div className="space-y-6">
          ...
          {previewUrl && (
            <div className="flex justify-center p-4 bg-slate-100 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
              <img src={previewUrl} alt="Converted PNG preview" className="max-h-64 object-contain rounded-xl shadow-md" />
            </div>
          )}
